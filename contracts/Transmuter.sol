...
// Forked from alchemix-finance: https://github.com/alchemix-finance/alchemix-protocol 
/**
...
contract Transmuter is Context {
    ...
    address public OcToken;
    address public Token;

    mapping(address => uint256) public depositedOcTokens;
    ...
    uint256 public totalSupplyOcTokens;
    uint256 public buffer;
    uint256 public lastDepositBlock;

    ///@dev values needed to calculate the distribution of base asset in proportion for ocTokens staked
    uint256 public pointMultiplier = 10e18;

    uint256 public totalDividendPoints;
    uint256 public unclaimedDividends;

    /// @dev occultist addresses whitelisted
    ...
    constructor(address _OcToken, address _Token, address _governance) public {
        require(_governance != ZERO_ADDRESS, "Transmuter: 0 gov");
        governance = _governance;
        OcToken = _OcToken;
        Token = _Token;
        TRANSMUTATION_PERIOD = 1;
    }

    ///@return displays the user's share of the pooled ocTokens.
    function dividendsOwing(address account) public view returns (uint256) {
        uint256 newDividendPoints = totalDividendPoints.sub(lastDividendPoints[account]);
        return depositedOcTokens[account].mul(newDividendPoints).div(pointMultiplier);
    }
    ...
    ///@dev Withdraws staked ocTokens from the transmuter
    ///
    /// This function reverts if you try to draw more tokens than you deposited
    ///
    ///@param amount the amount of ocTokens to unstake
    function unstake(uint256 amount) public updateAccount(msg.sender) {
        ...
        address sender = msg.sender;
        require(depositedOcTokens[sender] >= amount,"Transmuter: unstake amount exceeds deposited amount");
        depositedOcTokens[sender] = depositedOcTokens[sender].sub(amount);
        totalSupplyOcTokens = totalSupplyOcTokens.sub(amount);
        IERC20Burnable(OcToken).safeTransfer(sender, amount);
    }
    ///@dev Deposits ocTokens into the transmuter 
    ///
    ///@param amount the amount of ocTokens to stake
    function stake(uint256 amount)
        ...
        checkIfNewUser()
    {
        // requires approval of OcToken first
        address sender = msg.sender;
        //require tokens transferred in;
        IERC20Burnable(OcToken).safeTransferFrom(sender, address(this), amount);
        totalSupplyOcTokens = totalSupplyOcTokens.add(amount);
        depositedOcTokens[sender] = depositedOcTokens[sender].add(amount);
    }
    /// @dev Converts the staked ocTokens to the base tokens in amount of the sum of pendingdivs and tokensInBucket
    ///
    /// once the ocToken has been converted, it is burned, and the base token becomes realisedTokens which can be recieved using claim()    
    ///
    ...
    function transmute() public runPhasedDistribution() updateAccount(msg.sender) {
        ...
        if (pendingz > depositedOcTokens[sender]) {
            diff = pendingz.sub(depositedOcTokens[sender]);

            // remove overflow
            pendingz = depositedOcTokens[sender];
        }

        // decrease octokens
        depositedOcTokens[sender] = depositedOcTokens[sender].sub(pendingz);

        // BURN OCTOKENS
        IERC20Burnable(OcToken).burn(pendingz);

        // adjust total
        totalSupplyOcTokens = totalSupplyOcTokens.sub(pendingz);
    ...
    /// @dev Executes transmute() on another account that has had more base tokens allocated to it than ocTokens staked.
    ///
        ...
        updateAccount(toTransmute)
    {
        ...
        // check restrictions
        require(
            pendingz > depositedOcTokens[toTransmute],
            "Transmuter: !overflow"
        );
        ...
        uint256 diff = pendingz.sub(depositedOcTokens[toTransmute]);

        // remove overflow
        pendingz = depositedOcTokens[toTransmute];

        // decrease octokens
        depositedOcTokens[toTransmute] = 0;
    ...
    /// @dev Transmutes and unstakes all ocTokens
    ///
    /// This function combines the transmute and unstake functions for ease of use
    function exit() public {
        transmute();
        uint256 toWithdraw = depositedOcTokens[msg.sender];
        unstake(toWithdraw);
    }

    /// @dev Transmutes and claims all converted base tokens.
    ///
    /// This function combines the transmute and claim functions while leaving your remaining ocTokens staked.
    function transmuteAndClaim() public {
        transmute();
        claim();
    }

    /// @dev Transmutes, claims base tokens, and withdraws ocTokens.
    ///
    /// This function helps users to exit the transmuter contract completely after converting their ocTokens to the base pair.
    ...
    /// @dev Distributes the base token proportionally to all ocToken stakers.
    ///
    /// This function is meant to be called by the Occultist contract for when it is sending yield to the transmuter. 
    ...
    /// @dev Allocates the incoming yield proportionally to all ocToken stakers.
    ///
    ...
    function increaseAllocations(uint256 amount) internal {
        if(totalSupplyOcTokens > 0 && amount > 0) {
            totalDividendPoints = totalDividendPoints.add(
                amount.mul(pointMultiplier).div(totalSupplyOcTokens)
            );
    ...
    {
        uint256 _depositedAl = depositedOcTokens[user];
        ...
        uint256 _pendingdivs = _toDistribute.mul(depositedOcTokens[user]).div(totalSupplyOcTokens);
        ...
        returns (address[] memory theUserList, uint256[] memory theUserData)
    {
        ...
        for (uint256 x = 0; x < delta; x += 1) {
            ...
            _theUserData[y] = depositedOcTokens[userList[i]];
            _theUserData[y + 1] = dividendsOwing(userList[i]).add(tokensInBucket[userList[i]]).add(_toDistribute.mul(depositedOcTokens[userList[i]]).div(totalSupplyOcTokens));
            y += 2;
            i += 1;
        }
