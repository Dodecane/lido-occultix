...
# @author Forked from banteg: https://github.com/banteg/lido-vault
# @license MIT
...
@external
def __init__(steth_address: address):
...
def __default__():
    ...
    """
    shares: uint256 = Lido(self.token).submit(patron, value=msg.value)
...
def deposit(_tokens: uint256 = MAX_UINT256, recipient: address = msg.sender) -> uint256:
    ...
    """
    tokens: uint256 = min(_tokens, ERC20(self.token).balanceOf(msg.sender))
    shares: uint256 = Lido(self.token).getSharesByPooledEth(tokens)
    self._mint(recipient, shares)
    assert ERC20(self.token).transferFrom(msg.sender, self, tokens)
    return shares
...
def withdraw(_shares: uint256 = MAX_UINT256, recipient: address = msg.sender) -> uint256:
    ...
    tokens: uint256 = Lido(self.token).getPooledEthByShares(shares)
    self._burn(msg.sender, shares)
    assert ERC20(self.token).transfer(recipient, tokens)
    return tokens
...
def pricePerShare() -> uint256:
    ...
    """
    return Lido(self.token).getPooledEthByShares(10 ** self.decimals)
