...
contract Occultist is  ReentrancyGuard {
  ...
  constructor(
    ...
    public
  {
    require(_governance != ZERO_ADDRESS, "Occultist: governance address cannot be 0x0.");
    require(_sentinel != ZERO_ADDRESS, "Occultist: sentinel address cannot be 0x0.");

    token = _token;
    xtoken = _xtoken;
    governance = _governance;
    sentinel = _sentinel;
    flushActivator = 1 ether;// change for non 18 digit tokens
  ...
  function setPendingGovernance(address _pendingGovernance) external onlyGov {
    require(_pendingGovernance != ZERO_ADDRESS, "Occultist: governance address cannot be 0x0.");
  ...
  function setTransmuter(address _transmuter) external onlyGov {
    ...
    require(_transmuter != ZERO_ADDRESS, "Occultist: transmuter address cannot be 0x0.");
  ...
  function setRewards(address _rewards) external onlyGov {
    ...
    // transfers to the address because of `safeTransfer` checks.
    require(_rewards != ZERO_ADDRESS, "Occultist: rewards address cannot be 0x0.");
  ...
  function setHarvestFee(uint256 _harvestFee) external onlyGov {
    ...
    require(_harvestFee <= PERCENT_RESOLUTION, "Occultist: harvest fee above maximum.");
  ...
  function setCollateralizationLimit(uint256 _limit) external onlyGov {
    ...
    require(_limit >= MINIMUM_COLLATERALIZATION_LIMIT, "Occultist: collateralization limit below minimum.");
    require(_limit <= MAXIMUM_COLLATERALIZATION_LIMIT, "Occultist: collateralization limit above maximum.");
  ...
  function initialize(IVaultAdapter _adapter) external onlyGov {
    ...
    require(!initialized, "Occultist: already initialized");

    require(transmuter != ZERO_ADDRESS, "Occultist: cannot initialize transmuter address to 0x0");
    require(rewards != ZERO_ADDRESS, "Occultist: cannot initialize rewards address to 0x0");
  ...
  ///
  /// @param _amount the amount of occultic tokens to borrow.
  function mint(uint256 _amount) external nonReentrant noContractAllowed onLinkCheck expectInitialized {
    ...
    if (_totalCredit < _amount) {
      ...
      _cdp.checkHealth(_ctx, "Occultist: Loan-to-value ratio breached");
    } else {
  ...
  /// @dev Get the total amount of occultic tokens borrowed from a CDP.
  ///
  /// @param _account the user account of the CDP to query.
  ///
  ...
  modifier expectInitialized() {
    require(initialized, "Occultist: not initialized.");
    _;
  }
  ...
  ///
  modifier onlyGov() {
    require(msg.sender == governance, "Occultist: only governance.");
    _;
  }
  ...
  function _updateActiveVault(IVaultAdapter _adapter) internal {
    require(_adapter != IVaultAdapter(ZERO_ADDRESS), "Occultist: active vault address cannot be 0x0.");
    require(_adapter.token() == token, "Occultist: token mismatch.");
  ...
  function _recallFunds(uint256 _vaultId, uint256 _amount) internal returns (uint256, uint256) {
    require(emergencyExit || msg.sender == governance || _vaultId != _vaults.lastIndex(), "Occultist: not an emergency, not governance, and user does not have permission to recall funds from active vault");
