...
/// @title OcToken
///
/// @dev This is the contract for the Occultix utillity token usd.
///
...
contract OcToken is AccessControl, ERC20("Occultix stETH", "ocstETH") {
    ...
    event Paused(address occultistAddress, bool isPaused);
    ...
    modifier onlyWhitelisted() {
        require(whiteList[msg.sender], "ocstETH: Occultist is not whitelisted");
        _;
    }
    ...
    function mint(address _recipient, uint256 _amount)
        ...
        onlyWhitelisted
    {
        require(!blacklist[msg.sender], "ocstETH: Occultist is blacklisted.");
        uint256 _total = _amount.add(hasMinted[msg.sender]);
        require(
            _total <= ceiling[msg.sender],
            "ocstETH: Occultist's ceiling was breached."
        );
        require(!paused[msg.sender], "ocstETH: user is currently paused.");
    ...
    /// This function reverts if the caller does not have the admin role.
    function pauseOccultist(address _toPause, bool _state)
        external
        onlySentinel
    {
